...
let allMeals = document.querySelector(".products")
...
function showProduct(data) {
    allMeals.innerHTML = data.map((item) => {
        return `
        <div onclick="goToMealId('${item.idMeal}')" class="product">
...
function goToMealId(id) {
    window.location.href = `cocktail.html?id=${id}`
}
...
get_flags.innerHTML = flags.map((x) => {
    return `
    <img onclick="setFlag('${x.strArea}')" src="https://www.themealdb.com/images/icons/flags/big/64/${x.item}.png" >
    `
}).join("") 
